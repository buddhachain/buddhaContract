...
void Buddha::make_proposal(){
    ...
    //获取所有的基金会成员
    xchain::json vfounder ;
    if(!_scan_founder(vfounder) ) {
    ...
    for(int i = 0 ; i < vfounder.size() ; i++) {
        std::string id = vfounder.at(i)["id"].template get<std::string>();
