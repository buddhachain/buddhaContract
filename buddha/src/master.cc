...
void Buddha::apply_master(){
    ...
    _log_ok(__FILE__, __FUNCTION__, __LINE__, ent.to_string() + " apply  master over, please wait for approve .");
}
...
void Buddha::approve_master() {
    ...
    //判断是否是法师
    if( _is_master(id)) {
        _log_error(__FILE__, __FUNCTION__, __LINE__,ent.to_string() + " is already master .");
        return ;
    }
    ...
    _log_ok(__FILE__, __FUNCTION__, __LINE__, "approve master " + ent.to_string() + " success .");
}
...
void Buddha::recusal_master() {
    ...
    _log_ok(__FILE__, __FUNCTION__, __LINE__, "recusal master " + ent.to_string() + " success .");
}
