...
bool Main::_is_meritranking_exist(meritranking& ent, const string& ranking) {
...
bool Main::_scan_meritranking(xchain::json& ja, const string& cond) {
...
bool Main::_scan_meritranking(vector<meritranking>& v, const string& cond) {
    auto it = get_meritranking_table().scan({{"id",cond}});
    while(it->next() ) {
        meritranking ent;
        if (!it->get(&ent) ) {
...
bool Main::_add_meritranking(const string& id, const int64_t value ) {
...
// bool Main::_delete_meritranking_record(const string& id) {
//     meritranking ent;
